...
from kortex_examples.vision_detection import VisionDetection
from kortex_examples.depth_logic import DepthLogic  
from kortex_examples.motion_planner import MotionPlanner
...
class RoboticTypingController:
    ...
    def __init__(self):
        ...
        self.move_group = self.motion_planner.move_group
    ...
    def process_keyboard_detection(self):
        ...
        try:
            ...
            # rospy.loginfo(f"debug keypoints_3d_camera: {keypoints_3d_camera}")

            if not keypoints_3d_camera:
                rospy.loginfo(5, "No 3D positions calculated - check depth image")
                return
            ...
            for key, pos_3d_cam in keypoints_3d_camera.items():
                ...
                # rospy.loginfo(f"Transformed 3D position for key '{key}': {pos_3d_base}")

                # validate workspace has some bugs so it should be rectified and then used
                ...
                if pos_3d_base is not None:
                    ...
                    # rospy.loginfo(f"Valid 3D position for key '{key}': {pos_3d_base}")
                    ...
                    # rospy.loginfo("hello from keypoints_3d_base")



                    # rospy.loginfo(f"keypoints_3d_base[{key}] = {keypoints_3d_base[key]}")
                    ...
                    rospy.loginfo_throttle(5, 
                        ...
                        # f"base frame coordinate of v: {keypoints_3d_base['V']['position']}"

                    )
                else:
                    rospy.loginfo_throttle(10, f"❌ Key '{key}' out of workspace or transform failed")
    ...
    def get_robot_status(self):
        try:
            ...
            # self.pose_from_moveit = self.motion_planner.pose
